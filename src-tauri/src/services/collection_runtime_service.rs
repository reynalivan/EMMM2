...
pub async fn get_collection_runtime_descriptor(
    ...
    let runtime = collection_runtime_repo::get_tx(&mut tx, game_id).await?;
    let active_collection = match runtime
        .as_ref()
        .and_then(|state| state.active_collection_id.as_deref())
    {
        Some(id) => collection_repo::get_by_id_tx(&mut tx, id).await?,
        None => None,
    }
    .filter(|collection| collection.game_id == game_id && !collection.is_draft);
    let live_summary =
        crate::services::collection_service::load_live_runtime_summary_tx(&mut tx, game_id)
            .await
            .map_err(RuntimeStateError::from)?;
    let is_current = match active_collection.as_ref() {
        Some(collection) => {
            crate::services::collection_service::live_runtime_matches_collection_tx(
                &mut tx,
                game_id,
                &collection.id,
            )
    ...
    let missing_count = match active_collection.as_ref() {
        Some(collection) => {
            crate::services::collection_service::missing_collection_member_count_tx(
                &mut tx,
                game_id,
                &collection.id,
            )
...
pub async fn get_collection_runtime_state(
    ...
    let mods_path = crate::services::collection_service::load_game_mods_path(pool, game_id)
        .await
        .map_err(RuntimeStateError::from)?;
    let (current_mods, current_objects) =
        crate::services::collection_service::load_live_runtime_state(pool, game_id)
            .await
            .map_err(RuntimeStateError::from)?;
    let is_safe = crate::services::collection_service::live_runtime_is_safe(pool, game_id)
        .await
        .map_err(RuntimeStateError::from)?;
    ...
    let active_collection = match runtime
        ...
        .and_then(|state| state.active_collection_id.as_deref())
    {
        Some(id) => collection_repo::get_by_id(pool, id).await?,
        None => None,
    }
    .filter(|collection| collection.game_id == game_id && !collection.is_draft);
    let active_collection_id = active_collection
    ...
    let missing_count = match active_collection.as_ref() {
        Some(collection) => {
            crate::services::collection_service::load_projected_collection_state(
                pool,
                collection,
                mods_path.as_deref(),
            )
