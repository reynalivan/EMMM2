...
pub async fn commit_import_batch(
    ...
    let mut batch = crate::repo::import_batch::get_batch(pool, &input.batch_id)
        .await?
    ...
    let mods_root = crate::repo::game::get_mod_path(pool, &batch.game_id)
        .await?
        ...
        .any(|item| requested_ids.contains(&item.id) && item.status == ImportItemStatus::Committing)
    {
        ...
        .await?;
        batch = crate::repo::import_batch::get_batch(pool, &input.batch_id)
            .await?
            .ok_or_else(|| AppError::NotFound(format!("Import batch '{}'", input.batch_id)))?;
    }
    ...
    if selected_ids.is_empty() {
        crate::repo::import_batch::finish_batch_from_items(pool, &batch.id).await?;
    ...
    if !crate::repo::import_batch::begin_batch_commit(pool, &batch.id, &selected_id_list)
        .await?
    {
    ...
    for plan in &plans {
        crate::repo::import_batch::set_commit_item_state(
            pool,
            &plan.item.id,
    ...
    if let Err(move_error) = mutation_result {
        ...
        for plan in &plans {
            if rollback_succeeded {
                crate::repo::import_batch::restore_item_after_rollback(
                    pool,
                    &plan.item.id,
    ...
    for plan in &plans {
        ...
        crate::repo::import_batch::bind_reconciled_destination(
            pool,
            &plan.item.id,
            &object_id,
            &mod_id,
...
async fn resume_committed_items(
    ...
    if !crate::repo::import_batch::prepare_items_for_recovery(pool, &batch.id, &ids).await? {
        return Err(AppError::Validation(
...
async fn resolve_recovery_object_id(
    ...
    crate::repo::object::get_object_id_by_folder_key(&mut connection, &batch.game_id, &key)
        .await?
        .ok_or_else(|| {
            AppError::Internal(format!(
...
async fn resolve_object_dir(
    ...
    if let Some(object_id) = &item.destination_object_id {
        ...
        let (game_id, folder_path) =
            crate::repo::object::get_game_id_and_folder_path(pool, object_id)
                .await?
...
async fn resolve_reconciled_object_id(
    ...
    let mut connection = pool.acquire().await?;
    crate::repo::object::get_object_id_by_folder_key(&mut connection, game_id, &key)
        .await?
        .ok_or_else(|| {
            AppError::Internal(format!(
                "Disk reconcile did not create an object for '{}'",
...
async fn resolve_reconciled_mod_id(
    ...
    crate::repo::mods::get_mod_id_and_object_id_by_path(
        pool,
...
pub(super) async fn finalize_ready_to_move_archives(
    ...
    let Some(batch) = crate::repo::import_batch::get_batch(pool, batch_id).await? else {
        return Ok(());
    };
    ...
    for (source_path, items) in groups {
        ...
        let state = crate::repo::import_batch::get_mod_inbox_source_processing_state(
            pool,
            batch_id,
            &source_path,
        )
        ...
        if !retains_source {
            crate::repo::import_batch::complete_mod_inbox_source_processing(
                pool,
                batch_id,
                &source_path,
                None,
...
async fn finalize_batch_after_metadata(
    ...
    if batch.flow == ImportFlow::ReadyToMove {
        ...
        crate::repo::import_batch::complete_ready_to_move_archive_pending(pool, &batch.id)
            .await?;
    }
    let final_status =
        crate::repo::import_batch::finish_batch_from_items(pool, &batch.id).await?;
