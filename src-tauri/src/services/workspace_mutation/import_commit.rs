...
use crate::services::scanner::deep_matcher::{EntryKind, MasterDb};
...
fn fingerprint_path_key(path: &str) -> String {
    ...
    crate::common::path_key::folder_path_key(&resolved.to_string_lossy(), None)
}
...
pub fn rollback_move_journal(journal: &[MoveJournalEntry]) -> Result<(), AppError> {
    ...
    for entry in journal.iter().rev() {
        ...
        if let Err(error) = crate::services::fs_utils::file_utils::rename_cross_drive_fallback(
            &entry.target,
            &entry.source,
        ) {
            failures.push(format!(
...
pub async fn commit_import_batch(
    ...
    let mut batch = crate::repo::import_batch_repo::get_batch(pool, &input.batch_id)
        .await?
    ...
    let mods_root = crate::repo::game_repo::get_mod_path(pool, &batch.game_id)
        .await?
        ...
        .any(|item| requested_ids.contains(&item.id) && item.status == ImportItemStatus::Committing)
    {
        ...
        .await?;
        batch = crate::repo::import_batch_repo::get_batch(pool, &input.batch_id)
            .await?
            .ok_or_else(|| AppError::NotFound(format!("Import batch '{}'", input.batch_id)))?;
    }
    ...
    if selected_ids.is_empty() {
        crate::repo::import_batch_repo::finish_batch_from_items(pool, &batch.id).await?;
    ...
    crate::services::disk_reconcile::emit::ensure_mutation_preflight(app, pool, &batch.game_id)
        .await?;
    let operation_lock = app
        .try_state::<crate::services::fs_utils::operation_lock::OperationLock>()
    ...
    if !crate::repo::import_batch_repo::begin_batch_commit(pool, &batch.id, &selected_id_list)
        .await?
    {
    ...
    for plan in &plans {
        crate::repo::import_batch_repo::set_commit_item_state(
            pool,
            &plan.item.id,
    ...
    if let Err(move_error) = mutation_result {
        ...
        for plan in &plans {
            if rollback_succeeded {
                crate::repo::import_batch_repo::restore_item_after_rollback(
                    pool,
                    &plan.item.id,
    ...
    let path_hints = plans
        ...
        .filter_map(|plan| {
            plan.item.destination_object_id.as_ref().map(|object_id| {
                crate::services::mods::organizer_move::OrganizerMovePathHint {
        ...
        .collect();
    let reconcile =
        crate::services::disk_reconcile::emit::run_internal_disk_reconcile_with_path_hints_under_lease(
            app,
            pool,
            &batch.game_id,
            changed_paths,
    ...
    for plan in &plans {
        ...
        crate::repo::import_batch_repo::bind_reconciled_destination(
            pool,
            &plan.item.id,
            &object_id,
            &mod_id,
...
async fn apply_item_classification(
    ...
    item: &ImportItem,
    object_id: String,
) -> Result<crate::services::objects::classification::ClassificationWriteResult, AppError> {
    crate::services::objects::classification::apply_object_classification(
        pool,
        crate::services::objects::classification::ObjectClassificationInput {
            ...
            canonical_match: item.selected_entry_key.as_ref().map(|entry_key| {
                crate::services::objects::classification::CanonicalClassificationMatch {
            ...
            confirmed_source_alias: item.selected_entry_key.as_ref().map(|_| {
                crate::services::match_engine::inspection::source_display_name(&item.planned_name)
            }),
        },
    )
    .await
...
async fn recover_interrupted_commits(
    app: &tauri::AppHandle,
    pool: &sqlx::SqlitePool,
    batch: &crate::services::import_batch::types::ImportBatch,
    ...
    for (item, source, target) in &paths {
        match (source.exists(), target.exists()) {
            ...
            (false, true) => {
                ...
                crate::services::fs_utils::file_utils::rename_cross_drive_fallback(target, source)?;
...
async fn resume_committed_items(
    ...
    if !crate::repo::import_batch_repo::prepare_items_for_recovery(pool, &batch.id, &ids).await? {
        return Err(AppError::Validation(
    ...
    if let Some(active_lease) = lease.as_ref() {
        ...
        let hints = items
            ...
            .collect();
        let reconcile = crate::services::disk_reconcile::emit::run_internal_disk_reconcile_with_path_hints_under_lease(
            app,
            pool,
            &batch.game_id,
            changed_paths,
...
async fn resolve_recovery_object_id(
    ...
    let key =
        crate::common::path_key::folder_path_key(&object_dir.to_string_lossy(), Some(mods_root));
    let mut connection = pool.acquire().await?;
    crate::repo::object_repo::get_object_id_by_folder_key(&mut connection, &batch.game_id, &key)
        .await?
        .ok_or_else(|| {
            AppError::Internal(format!(
...
fn cleanup_import_staging(app: &tauri::AppHandle, batch_id: &str) -> Result<(), AppError> {
    ...
    crate::services::import_batch::staging::cleanup_batch_staging(&root, batch_id)?;
    Ok(())
}

fn physical_import_name(planned_name: &str) -> String {
    crate::services::mods::core_ops::standardize_prefix(planned_name, false)
}
...
async fn resolve_plan(
    ...
    crate::services::mods::core_ops::validate_folder_name_component(&physical_name)?;
...
async fn resolve_object_dir(
    ...
    if let Some(object_id) = &item.destination_object_id {
        ...
        let (game_id, folder_path) =
            crate::repo::object_repo::get_game_id_and_folder_path(pool, object_id)
                .await?
        ...
        crate::services::mods::core_ops::validate_folder_name_component(&entry.name)?;
...
fn validate_unique_targets(plans: &[PlannedMove]) -> Result<(), AppError> {
    ...
    for plan in plans {
        let key = crate::common::path_key::folder_path_key(&plan.target.to_string_lossy(), None);
...
fn validate_preview_fingerprint(plan: &PlannedMove) -> Result<(), AppError> {
    ...
    let current = crate::services::match_engine::inspection::inspect_source(
        &crate::services::match_engine::inspection::InspectionRequest {
...
async fn execute_moves(
    ...
    for plan in plans {
        ...
        match crate::services::fs_utils::file_utils::rename_cross_drive_fallback_tracked(
            &plan.source,
            &plan.target,
        ) {
...
async fn resolve_reconciled_object_id(
    ...
    let key = crate::common::path_key::folder_path_key(
        &plan.object_dir.to_string_lossy(),
        Some(mods_root),
    );
    let mut connection = pool.acquire().await?;
    crate::repo::object_repo::get_object_id_by_folder_key(&mut connection, game_id, &key)
        .await?
        .ok_or_else(|| {
            AppError::Internal(format!(
                "Disk reconcile did not create an object for '{}'",
...
async fn resolve_reconciled_mod_id(
    ...
    crate::repo::mod_repo::get_mod_id_and_object_id_by_path(
        pool,
...
pub(super) async fn finalize_ready_to_move_archives(
    ...
    let Some(batch) = crate::repo::import_batch_repo::get_batch(pool, batch_id).await? else {
        return Ok(());
    };
    ...
    for (source_path, items) in groups {
        ...
        let state = crate::repo::import_batch_repo::get_mod_inbox_source_processing_state(
            pool,
            batch_id,
            &source_path,
        )
        ...
        if !retains_source {
            crate::repo::import_batch_repo::complete_mod_inbox_source_processing(
                pool,
                batch_id,
                &source_path,
                None,
...
async fn finalize_batch_after_metadata(
    ...
    if batch.flow == ImportFlow::ReadyToMove {
        ...
        crate::repo::import_batch_repo::complete_ready_to_move_archive_pending(pool, &batch.id)
            .await?;
    }
    let final_status =
        crate::repo::import_batch_repo::finish_batch_from_items(pool, &batch.id).await?;
    if final_status == crate::services::import_batch::types::ImportBatchStatus::Done {
