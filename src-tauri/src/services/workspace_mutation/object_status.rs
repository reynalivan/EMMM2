...
use crate::services::workspace_mutation::import_commit::{rollback_move_journal, MoveJournalEntry};
...
pub async fn disable_object_roots(
    ...
    let mods_root_raw = crate::repo::game_repo::get_mod_path(pool, game_id)
        .await?
    ...
    for object_id in object_ids {
        ...
        let source = crate::services::mods::core_ops::resolve_existing_runtime_variant(
        ...
        let target = source.with_file_name(crate::services::mods::core_ops::standardize_prefix(
            name, false,
        ));
        if source == target {
            continue;
    ...
    crate::services::disk_reconcile::emit::ensure_mutation_preflight(app, pool, game_id).await?;
