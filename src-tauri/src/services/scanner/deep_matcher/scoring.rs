...
pub fn has_primary_evidence(reasons: &[Reason]) -> bool {
    reasons.iter().any(|reason| match reason {
        ...
        | Reason::IniContentToken { .. } => true,
        Reason::DirectNameSupport { .. }
        | Reason::TokenOverlap { .. }
        | Reason::AiRerank { .. }
        | Reason::NegativeEvidence { .. } => false,
    })
}
...
fn push_reason_capped(state: &mut ScoreState, reason: Reason) {
...
fn reason_priority(reason: &Reason) -> usize {
    match reason {
        ...
        Reason::NegativeEvidence { .. } => 2,
        Reason::TokenOverlap { .. } => 3,
        Reason::DeepNameToken { .. } => 4,
        Reason::IniSectionToken { .. } => 5,
        Reason::IniContentToken { .. } => 6,
