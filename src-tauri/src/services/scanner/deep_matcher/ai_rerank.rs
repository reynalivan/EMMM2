...
pub trait AiRerankProvider {
    fn rerank(&self, request: &AiRerankRequest) -> HashMap<usize, f32>;
}
...
pub fn maybe_apply_ai_rerank(
    ...
    let ai_scores = resolve_ai_scores(config.cache, provider, &request);
...
fn resolve_ai_scores(
    ...
    let scores: HashMap<usize, f32> = provider
        .rerank(request)
        .into_iter()
        .map(|(entry_id, score)| (entry_id, score.clamp(0.0, 1.0)))
        .collect();
