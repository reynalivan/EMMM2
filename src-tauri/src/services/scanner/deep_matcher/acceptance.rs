...
use super::quick_pipeline_result::{build_evidence, empty_evidence};
use super::types::{
...
pub fn try_stage_accept(
    ...
    observed_buckets: &super::stages::ObservedTokenBuckets,
...
fn assemble_ranked_result(
    ...
    if let Some(confidence) = best_confidence {
        if let Some(best_candidate) = candidates.first_mut() {
            best_candidate.confidence = confidence.clone();
        }
    }
    ...
    let foreign_hits = candidates
        ...
        .and_then(|c| {
            c.reasons.iter().find_map(|r| {
                if let super::types::Reason::NegativeEvidence {
                    foreign_strong_hits,
                } = r
                {
...
#[cfg(test)]
#[path = "acceptance_result_tests.rs"]
