...
pub async fn sync_objects_for_game(
    ...
    let p = std::path::Path::new(mods_path_str);
    if p.exists() && p.is_dir() {
        ...
        let current_objects_paths =
            crate::repo::object_repo::get_objects_folder_paths(pool, game_id).await?;
        ...
        if let Ok(mut tx) = pool.begin().await {
            ...
            for folder in &fs_folders {
                ...
                if !db_folders_normalized.contains(&canonical_name_key(&norm)) {
                    changes = true;
                    let folder_path_str = folder.clone();
                    let folder_name =
                        crate::services::scanner::core::normalizer::normalize_display_name(folder);
                    ...
                    // GAP FIX: Also create a mod row for the newly discovered folder
                    // This ensures "Quick Import" (silent indexer) actually populates the Mod List.
                    let mod_id = crate::services::scanner::sync::helpers::generate_stable_id(
                        game_id,
                        &folder_path_str,
                    );
                    ...
                    // Call mod_repo::insert_new_mod
                    let _ = crate::repo::mod_repo::insert_new_mod(
                        &mut *tx,
                        &mod_id,
                        game_id,
                        &resolved_obj_id,
                    ...
                    if !resolved_obj_id.is_empty() {
                        log::info!(
                            "sync_objects_for_game: Processed folder '{}' -> Object/Mod '{}'",
                            folder_path_str,
                            folder_name
                        );
                    } else {
