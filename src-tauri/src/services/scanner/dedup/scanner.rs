...
pub async fn scan_duplicates(
    ...
    let mod_rows = crate::repo::mods::get_all_mods_id_and_paths_tx(&mut conn, game_id).await?;
    drop(conn);
...
async fn fetch_whitelist_pairs(
    ...
    let rows = crate::repo::dedup::get_duplicate_whitelist_pairs(db, game_id).await?;
