...
use crate::services::disk_reconcile::orchestrator::DiskReconcilePathHint;
...
use crate::services::disk_reconcile::rename_confirmation::detect_rename_confirmations;
...
pub struct ReconcileOutcome {
    ...
    pub rename_confirmations: Vec<crate::services::disk_reconcile::types::RenameConfirmationGroup>,
...
pub struct ReconcileDiskProjectionRequest<'a> {
    ...
    pub progress_reporter:
        Option<Arc<crate::services::disk_reconcile::orchestrator::DiskReconcileProgressReporter>>,
}
...
fn internal_mutation_rename_evidence(
    ...
    for changed_path in changed_paths {
        ...
        let key = crate::common::path_key::folder_path_key(&relative.to_string_lossy(), None);
...
pub async fn reconcile_disk_projection(
    ...
    for changed_path in changed_paths {
        let path = Path::new(changed_path);
        if crate::services::disk_reconcile::path_classifier::is_thumbnail_path(path) {
            crate::services::images::thumbnail_cache::ThumbnailCache::invalidate(path);
        }
    }
    ...
    let thumbnail_only_watcher_batch = matches!(reason, DiskReconcileReason::WatcherBatch)
        ...
        && changed_paths.iter().all(|path| {
            crate::services::disk_reconcile::path_classifier::is_thumbnail_path(Path::new(path))
        });
    ...
    if should_reconcile {
        ...
        let snapshot = tokio::task::spawn_blocking(move || {
            let on_progress = |progress: DiskSnapshotProgress| {
                if let Some(reporter) = &snapshot_progress {
                    reporter.emit(
                        crate::services::disk_reconcile::types::DiskReconcilePhase::ScanningRoots,
        ...
        if force_full {
            crate::services::images::thumbnail_cache::ThumbnailCache::clear_memory();
        ...
        if let Some(reporter) = &progress_reporter {
            reporter.emit(
                crate::services::disk_reconcile::types::DiskReconcilePhase::Projecting,
                0,
                None,
                None,
            );
        ...
        let projection = crate::services::disk_reconcile::disk_snapshot::DiskProjection {
            objects: projection
                .objects
                .iter()
