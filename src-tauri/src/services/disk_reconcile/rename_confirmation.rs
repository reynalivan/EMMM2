...
fn scope_key(kind: &RenameConfirmationKind, path: &str) -> String {
    match kind {
        ...
        RenameConfirmationKind::Mod => path
            ...
            .map(|segment| crate::common::path_key::folder_path_key(segment, None))
            .unwrap_or_default(),
    }
}
...
fn detect_rename_confirmations_with_protection_from_rows(
    ...
    let previous_objects = objects
        ...
        .map(|entry| RuntimeEntry {
            ...
            logical_key: crate::common::path_key::folder_path_key(&entry.folder_path, None),
...
fn path_is_covered(path: &str, evidence_root: &str) -> bool {
    path.eq_ignore_ascii_case(evidence_root)
        || crate::common::path_key::strip_path_prefix_preserve_display(path, evidence_root, None)
            .is_some()
}
...
fn validate_explicit_resolutions(
    ...
    mods_path: &std::path::Path,
    watcher_events: &[ModWatchEvent],
) -> Result<bool, crate::domain::errors::AppError> {
    ...
    for (group_id, from, to, apply_as_rename) in resolutions {
        ...
        let group = groups
            ...
            .ok_or_else(|| {
                crate::domain::errors::AppError::Validation(
...
mod tests {
    ...
    use crate::services::disk_reconcile::disk_snapshot::{DiskModEntry, DiskObjectEntry};
    ...
    fn disk_mod(path: &str, identity: Option<&str>) -> DiskModEntry {
        let object_key = PathBuf::from(path)
            ...
            .next()
            .map(|part| {
                crate::common::path_key::folder_path_key(&part.as_os_str().to_string_lossy(), None)
            })
            .unwrap();
        DiskModEntry {
            folder_path: path.to_string(),
