...
use crate::repo::utils::stable_ids::generate_stable_id;
...
async fn seed_collection_with_members(pool: &sqlx::SqlitePool, mod_paths: &[&str]) {
    ...
    for mod_path in mod_paths {
        ...
        let object_ref_key = crate::common::path_key::folder_path_key(&object_folder, None);
        sqlx::query(
        ...
        .bind(&mod_id)
        .bind(mod_path)
        .bind(crate::common::path_key::folder_path_key(mod_path, None))
        .bind(&object_ref_key)
        .bind(&object_id)
        .execute(pool)
        .await
...
async fn full_reconcile_restores_object_name_after_staging_a_stale_path_key() {
    ...
    assert_eq!(row.1, crate::common::path_key::canonical_name_key("Alice"));
    assert_eq!(row.2, "Alice");
    assert_eq!(
        row.3,
        crate::common::path_key::folder_path_key("Alice", None)
    );
}
...
async fn full_reconcile_repairs_an_object_name_leaked_by_an_earlier_stage() {
    ...
    sqlx::query("UPDATE objects SET name = ?, name_key = ? WHERE game_id = ?")
        .bind(leaked_stage)
        .bind(crate::common::path_key::canonical_name_key(leaked_stage))
        .bind("game-1")
        .execute(&ctx.pool)
        .await
...
async fn parent_delete_preserves_collection_members_and_reports_every_child_missing() {
    ...
    .bind("mod-red")
    .bind("Bob/Red")
    .bind(crate::common::path_key::folder_path_key("Bob/Red", None))
    .bind("bob")
    .bind("obj-bob")
    .bind("FlatModRoot")
    .execute(&ctx.pool)
    ...
    let snapshot = crate::services::projected_state::parse_snapshot_json(&snapshot_json)
...
async fn full_reconcile_heals_offline_nested_terminal_rename_by_filesystem_identity() {
    ...
    assert_eq!(
        crate::common::path_key::folder_path_key(&collection_path, None),
        crate::common::path_key::folder_path_key(&expected_new_path, None)
    );
    assert!(run.path_updates.iter().any(|update| {
        crate::common::path_key::folder_path_key(&update.from, None).ends_with("variants/old")
            && crate::common::path_key::folder_path_key(&update.to, None).ends_with("variants/new")
    }));
...
async fn full_reconcile_heals_offline_parent_rename_and_all_collection_children() {
    ...
    sqlx::query("INSERT INTO collection_mods (collection_id, mod_id, mod_path, mod_path_key, object_ref_key, object_id, node_type) VALUES ('collection-1', ?, 'Alice/Blue', ?, 'alice', ?, 'FlatModRoot')")
        .bind(&mod_id)
        .bind(crate::common::path_key::folder_path_key("Alice/Blue", None))
        .bind(&object_id)
        .execute(&ctx.pool)
        .await
...
async fn offline_rename_into_deleted_destination_keeps_source_row_metadata() {
    ...
    for (mod_id, mod_path, object_id) in members {
        let is_source =
            crate::common::path_key::folder_path_key(&mod_path, None).ends_with("alice/source");
    ...
    assert!(
        crate::common::path_key::folder_path_key(&rows[0].0, None).ends_with("alice/destination")
    );
    ...
    assert!(
        crate::common::path_key::folder_path_key(&collection_members[0].0, None)
    ...
    assert!(crate::common::path_key::folder_path_key(preview_path, None)
...
async fn offline_folder_swap_follows_both_filesystem_identities_without_id_collision() {
    ...
    assert!(crate::common::path_key::folder_path_key(&rows[0].0, None).ends_with("alice/first"));
    assert_eq!(rows[0].1, 0);
    assert!(crate::common::path_key::folder_path_key(&rows[1].0, None).ends_with("alice/second"));
