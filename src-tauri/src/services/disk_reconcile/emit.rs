...
pub(crate) fn conflicts_intersect_paths(
    conflicts: &[crate::services::disk_reconcile::types::FolderNameConflictGroup],
    paths: &[String],
) -> bool {
    paths.iter().any(|path| {
...
pub async fn run_full_internal_disk_reconcile_under_lease(
    ...
    pool: &sqlx::SqlitePool,
    game_id: &str,
    lease: &crate::services::disk_reconcile::orchestrator::DiskMutationLease,
) -> Result<crate::services::disk_reconcile::types::DiskReconcileResult, AppError> {
    let config = app
    ...
    crate::services::disk_reconcile::orchestrator::reconcile_disk_state_under_lease(
...
pub async fn run_internal_disk_reconcile_with_path_hints_under_lease(
    ...
    game_id: &str,
    changed_paths: Vec<String>,
    path_hints: Vec<crate::services::mods::organizer_move::OrganizerMovePathHint>,
    lease: &crate::services::disk_reconcile::orchestrator::DiskMutationLease,
) -> Result<crate::services::disk_reconcile::types::DiskReconcileResult, AppError> {
    let config = app
        .try_state::<crate::services::config::ConfigService>()
    ...
    let request = DiskReconcileRequest::manual_with_path_hints(
        ...
        path_hints
            .into_iter()
            .map(
                |hint| crate::services::disk_reconcile::orchestrator::DiskReconcilePathHint {
...
pub fn start_initial_disk_recovery(
    ...
    state: &DiskReconcileState,
    game_id: &str,
) -> crate::services::disk_reconcile::orchestrator::InitialRecoveryReadiness {
    use crate::services::disk_reconcile::orchestrator::{
...
async fn run_internal_disk_reconcile_with_options(
    ...
    path_hints: Vec<crate::services::disk_reconcile::orchestrator::DiskReconcilePathHint>,
) -> Result<crate::services::disk_reconcile::types::DiskReconcileResult, AppError> {
    let config = app
        .try_state::<crate::services::config::ConfigService>()
        .ok_or_else(|| {
...
pub async fn ensure_open_path_preflight(
    ...
    let census = tokio::task::spawn_blocking(move || {
        crate::services::disk_reconcile::disk_snapshot::collect_disk_identity_census(&census_path)
    })
    .await?
    .map_err(|error| AppError::Io(error.into_message()))?;
    let conflicts = crate::services::disk_reconcile::identity_conflicts::detect_folder_name_conflicts_from_census(
        game_id,
        &census,
    );
