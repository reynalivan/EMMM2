...
fn requested_runtime_effects(
    ...
    let startup_recovery = *reason == DiskReconcileReason::StartupBoot;

    PendingRuntimeEffects {
        collections_dirty: applied
            && (startup_recovery || folders_changed || objects_changed || runtime_file_changed),
        overlay_refresh: applied && (startup_recovery || folders_changed || runtime_file_changed),
    }
}
...
async fn finalize_runtime_effects(request: RuntimeEffectsRequest<'_>) -> DiskReconcileResult {
    ...
    let settlement = if applied {
        Some(
            crate::services::app::runtime_effects::settle_committed_runtime_effects(
...
pub(super) async fn run_refresh_once(
    ...
    crate::services::mods::core_ops::recover_folder_conflict_journals(
        &game.mod_path,
