...
pub struct ApplyGameModsDirectoryResult {
    pub game: crate::services::config::GameConfig,
    pub inspection: GameModsDirectoryInspection,
    pub reconcile: crate::services::disk_reconcile::types::DiskReconcileResult,
}
...
pub async fn apply_game_mods_directory(
    context: crate::services::disk_reconcile::orchestrator::DiskReconcileContext<'_>,
    request: ApplyGameModsDirectoryRequest,
    game_guard: &tokio::sync::MutexGuard<'_, ()>,
    operation_guard: &crate::services::fs_utils::operation_lock::OpGuard,
    ...
    let reconcile_result =
        crate::services::disk_reconcile::orchestrator::reconcile_disk_state_under_locks(
            context.clone(),
    ...
    let reconcile = match reconcile_result {
        Ok(result)
            if result.status
                != crate::services::disk_reconcile::types::DiskReconcileStatus::SourceUnavailable =>
        {
            result
        }
        Ok(result) => {
...
mod tests {
    ...
    fn object(path: &str, identity: Option<&str>) -> ReconcileObjectRow {
        ReconcileObjectRow {
            ...
            folder_path_key: crate::common::path_key::folder_path_key(path, None),
    ...
    async fn inspection_rejects_lossy_ini_decoding_before_any_apply_can_report_applied() {
        ...
        assert!(
            matches!(error, crate::domain::errors::AppError::Io(message) if message.contains("encoding"))
        );
    ...
    async fn apply_rejects_a_candidate_changed_after_inspection_without_updating_config() {
        ...
        let config = crate::services::config::ConfigService::new_for_test(context.pool.clone());
        ...
        let operation_lock = crate::services::fs_utils::operation_lock::OperationLock::new();
        ...
        let result = apply_game_mods_directory(
            crate::services::disk_reconcile::orchestrator::DiskReconcileContext {
                ...
                watcher_suppressor: std::sync::Arc::new(
                    crate::services::scanner::watcher::WatcherSuppressor::new(false),
                ),
        ...
        assert!(matches!(
            result,
            Err(crate::domain::errors::AppError::Validation(_))
        ));
        assert_eq!(
    ...
    async fn post_commit_runtime_effect_failure_does_not_restore_the_previous_source() {
        ...
        assert!(reconcile.warnings.iter().any(|warning| {
            warning.kind
                == crate::services::disk_reconcile::types::DiskReconcileWarningKind::RuntimeEffectsPending
        }));
    }
}
