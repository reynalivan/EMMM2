...
pub fn init_pool(app_data_dir: &std::path::Path) -> sqlx::SqlitePool {
    ...
    block_on(async {
        ...
        if let Err(e) = repo::utils::unicode_keys::ensure_unicode_keys(&p).await {
...
async fn recover_interrupted_transfers(pool: &sqlx::SqlitePool) {
    match repo::browser::fail_interrupted_downloads(pool).await {
    ...
    match repo::browser::fail_interrupted_jobs(pool).await {
...
pub fn run_startup_reconcile(app: tauri::AppHandle) {
    ...
    block_on(async {
        match repo::task::reclaim_interrupted_apply_tasks(&pool).await {
        ...
        match repo::task::purge_old_tasks(&pool).await {
        ...
        match repo::import_batch::recover_interrupted_batch_states(&pool).await {
        ...
        match repo::import_batch::list_terminal_batch_ids_for_staging_cleanup(&pool).await {
