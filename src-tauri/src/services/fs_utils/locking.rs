...
pub fn get_locking_processes(path: &Path) -> Vec<String> {
    ...
    unsafe {
        ...
        let mut proc_info: Vec<RM_PROCESS_INFO> = Vec::with_capacity(n_proc_info as usize);
        proc_info.set_len(n_proc_info as usize);
        ...
        ) == 0
        {
            ...
            for i in 0..n_proc_info as usize {
                let name_wide = &proc_info[i].strAppName;
