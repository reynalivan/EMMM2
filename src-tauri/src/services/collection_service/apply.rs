...
use crate::domain::task::{TaskStatus, TASK_TYPE_APPLY_COLLECTION};
use sqlx::SqlitePool;
...
async fn apply_collection_with_finalization(
    ...
    let final_active_collection_id = match finalization {
        ...
        ActiveCollectionFinalization::RequestDefault => {
            crate::repo::collection_runtime_repo::get(request.pool, request.game_id)
                .await?
                .and_then(|runtime| runtime.active_collection_id)
        }
    };
    let task_id = uuid::Uuid::new_v4().to_string();
    crate::repo::task_repo::create_claimed_task_with_final_active(
        request.pool,
        &task_id,
        request.game_id,
    ...
    let mut ctx = match prepare_apply_context(request, &task_id).await {
        Ok(ctx) => ctx,
        Err(error) => {
            let _ = crate::repo::task_repo::compare_and_set_status(
                pool,
                &task_id,
                TaskStatus::Running,
                TaskStatus::Failed,
...
async fn prepare_apply_context(
    ...
    crate::repo::task_repo::update_rollback_intent(
        request.pool,
        task_id,
