...
async fn apply_collection_converges_mixed_parent_and_child_state_to_clean() {
    ...
    let runtime = crate::services::collection_runtime_service::get_collection_runtime_state(
        &ctx.pool,
        "game-mixed-parent-child",
    )
    .await
    .expect("load runtime state");
    let target = collection_repo::get_by_id(&ctx.pool, &collection.id)
        .await
    ...
    assert_eq!(
        ...
        crate::domain::runtime_state::RuntimeStatus::Clean
    );
}
...
async fn active_pointer_failure_keeps_the_apply_task_pending_for_recovery() {
    ...
    for (id, name) in [("baseline", "Baseline"), ("target", "Target")] {
        let collection =
            collection_repo::create(&ctx.pool, id, "game-finalize-gap", name, true, false)
                .await
                .expect("create collection");
        persist_projected_state(
            &ctx.pool,
    ...
    let tasks = crate::repo::task_repo::get_all_pending_tasks_global(&ctx.pool)
        .await
...
async fn pending_apply_rejects_another_apply_before_draft_capture() {
    ...
    for (id, name, is_unsaved) in [
        ...
        collection_repo::create(&ctx.pool, id, "game-open-apply", name, true, is_unsaved)
            .await
            .expect("create collection");
        persist_projected_state(
            &ctx.pool,
    ...
    drop(connection);
    let original_signature = collection_repo::get_by_id(&ctx.pool, "existing-draft")
        .await
        .expect("load draft")
        .expect("draft exists")
        .signature;
    ...
    let current_signature = collection_repo::get_by_id(&ctx.pool, "existing-draft")
        .await
        .expect("reload draft")
        .expect("draft exists")
        .signature;
...
async fn assert_replace_failure_is_atomic(failpoint: ReplaceFailpoint) {
    ...
    let stored_state = projected_state_service::build_projected_state(
    ...
    let stored_paths: Vec<String> = collection_repo::get_mods(&ctx.pool, "replace-target")
        .await
        .expect("reload target members")
        .into_iter()
        .map(|member| member.mod_path)
        .collect();
    let runtime = crate::repo::collection_runtime_repo::get(&ctx.pool, "game-replace-atomic")
        .await
        .expect("reload runtime state")
        .expect("runtime state exists");
    let draft_exists = collection_repo::get_by_id(&ctx.pool, "replace-draft")
        .await
        .expect("reload draft")
        .is_some();
    let target_is_safe = collection_repo::get_by_id(&ctx.pool, "replace-target")
        .await
