use super::*;
use crate::repo::collection_runtime_repo;
...
async fn deleting_active_collection_clears_baseline_without_creating_a_draft() {
    ...
    collection_runtime_repo::set_active(&ctx.pool, "game-1", Some(&active.id))
        .await
    ...
    let runtime = collection_runtime_repo::get(&ctx.pool, "game-1")
        .await
        .expect("load runtime")
...
async fn deleting_inactive_collection_preserves_active_baseline_and_draft() {
    ...
    collection_runtime_repo::set_active_tx(&mut tx, "game-1", Some(&active.id))
        .await
        .expect("set active baseline");
    collection_runtime_repo::set_draft_tx(&mut tx, "game-1", &draft.id, Some(&active.id))
        .await
        .expect("set draft");
