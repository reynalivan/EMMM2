...
fn rewrite_descendant_path(path: &str, old_root: &str, new_root: &str) -> Option<String> {
    let path_key = crate::common::path_key::folder_path_key(path, None);
    let old_key = crate::common::path_key::folder_path_key(old_root, None);
    if path_key == old_key {
...
pub(crate) async fn stage_identity_path_transitions_tx(
    ...
    for rewrite in object_rewrites {
        let old_ref_key = crate::common::path_key::folder_path_key(&rewrite.from, None);
        let final_ref_key = crate::common::path_key::folder_path_key(&rewrite.to, None);
        ...
        for (id, name) in collection_repo::stage_object_reference(
            &mut *conn,
            game_id,
            &old_ref_key,
            &staged_ref_key,
        ...
        staged_objects.push((
            staged_ref_key,
            final_ref_key,
            crate::common::normalizer::normalize_display_name(&rewrite.to),
        ));
    }
    ...
    let exact_mod_rewrites = mod_rewrites
        ...
        .map(|rewrite| {
            (
                crate::common::path_key::folder_path_key(&rewrite.from, None),
                rewrite,
            )
        })
    ...
    for member in collection_repo::list_member_paths_for_game(&mut *conn, game_id).await? {
        let member_key = crate::common::path_key::folder_path_key(&member.mod_path, None);
        ...
        if crate::common::path_key::folder_path_key(&final_path, None) == member_key {
            continue;
        }
        ...
        collection_repo::stage_member_path(
            &mut *conn,
    ...
    for (staged_ref_key, final_ref_key, final_display_name) in staged_objects {
        collection_repo::finalize_staged_object_reference(
            &mut *conn,
            game_id,
            &staged_ref_key,
            &final_ref_key,
    ...
    for (collection_id, staged_path, final_path) in staged_members {
        collection_repo::finalize_staged_member_path(
            &mut *conn,
            &collection_id,
            &staged_path,
            &final_path,
...
pub async fn handle_object_renamed_tx(
    ...
    for old_candidate in unique_reference_candidates(old_object_folder) {
        let old_ref_key = crate::common::path_key::folder_path_key(&old_candidate, None);
        let new_ref_key = crate::common::path_key::folder_path_key(&new_logical_folder, None);
        for (id, name) in collection_repo::rewrite_object_references(
            &mut *conn,
            game_id,
            &old_ref_key,
            &new_ref_key,
            &crate::common::normalizer::normalize_display_name(&new_logical_folder),
        )
        .await?
        {
        ...
        for (old_sep, new_sep) in [
            ...
            let rows =
                collection_repo::find_mods_with_path_prefix(&mut *conn, game_id, &old_sep).await?;
...
async fn load_collection_references_tx(
    ...
    let references = collection_repo::get_references_by_mod_path(&mut *conn, game_id, mod_path)
        .await?
        .into_iter()
...
async fn recompute_signature_tx(
    ...
    let (_, mods_path) = collection_repo::get_projection_context(&mut *conn, collection_id).await?;
