...
pub(crate) fn validate_folder_base_name(name: &str) -> Result<(), AppError> {
    ...
    if crate::common::normalizer::is_disabled_folder(name) {
...
pub(crate) fn find_sibling_identity_collision(
    ...
    let target_identity = crate::common::normalizer::normalize_display_name(target_name);
    ...
    for entry in entries.flatten() {
        ...
        let identity_collision = entry_path.is_dir()
            && crate::common::normalizer::normalize_display_name(&entry_name)
