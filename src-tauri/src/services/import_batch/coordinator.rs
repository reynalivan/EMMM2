...
pub async fn refresh_import_item_suggestions(
    ...
    ini_filters: &crate::services::scanner::deep_matcher::analysis::content::PreparedTokenFilters,
) -> Result<ImportItem, AppError> {
    ...
    let mut canonical = crate::services::match_engine::canonical_match::match_canonical_objects(
    ...
    let canonical_identity = canonical.first().and_then(|suggestion| {
        master_db.entries.iter().find_map(|entry| {
            ...
            (key == suggestion.entry_key).then(|| {
                crate::services::match_engine::types::CanonicalIdentity {
                    entry_key: key,
    ...
    let destinations = crate::services::match_engine::destination::resolve_destination_candidates(
...
fn rerank_with_metadata(
    suggestions: &mut [crate::services::import_batch::types::CanonicalSuggestion],
    ...
    for suggestion in suggestions.iter_mut() {
        ...
        suggestion.confidence_tier =
            crate::services::import_batch::types::ConfidenceTier::from_percentage(
                suggestion.confidence_percentage,
            );
    }
...
fn planned_name_for_source(path: &Path, kind: ImportSourceKind) -> Result<String, AppError> {
    ...
    Ok(crate::services::mods::core_ops::standardize_prefix(
        &name, false,
    ))
}
