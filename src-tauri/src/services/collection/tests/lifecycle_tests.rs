...
async fn save_current_state_becomes_the_game_runtime_baseline() {
    ...
    let runtime = crate::repo::collection::runtime::get(&ctx.pool, "game-save-no-pointer")
        .await
        .expect("load runtime")
...
async fn collection_safety_summary_tracks_current_member_classification_without_dirtying_state() {
    ...
    let runtime = crate::services::collection_runtime::get_collection_runtime_state(
        &ctx.pool, "game-1",
    )
    .await
    .expect("load runtime");
...
async fn clone_snapshot_does_not_change_the_runtime_baseline() {
    ...
    let runtime = crate::repo::collection::runtime::get(&ctx.pool, "game-1")
        .await
        .expect("load runtime")
...
async fn clean_runtime_does_not_create_or_replace_last_changes() {
    ...
    let runtime = crate::repo::collection::runtime::get(&ctx.pool, "game-clean-draft")
        .await
...
async fn unsaved_runtime_creates_one_last_changes_draft_without_baseline() {
    ...
    let draft_id = capture_last_changes_if_needed(&ctx.pool, "game-unsaved-draft")
        ...
        .expect("draft id");
    let runtime = crate::repo::collection::runtime::get(&ctx.pool, "game-unsaved-draft")
        .await
        .expect("runtime state")
        .expect("runtime row");
...
async fn clear_last_changes_rejects_a_draft_referenced_by_an_open_apply() {
    ...
    let error =
        crate::services::collection::clear_last_changes(&ctx.pool, "game-clear-protected")
            .await
            .expect_err("open recovery task must protect its rollback draft");

    assert!(format!("{error}").contains("recovery"));
    let runtime = crate::repo::collection::runtime::get(&ctx.pool, "game-clear-protected")
        .await
        .expect("load runtime")
        .expect("runtime exists");
    assert_eq!(
