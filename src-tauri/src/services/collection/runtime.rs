...
pub async fn capture_last_changes_if_needed(
    ...
    let state = projected_state::build_projected_state(
        &mods,
        &objects,
    ...
    if let Some(base) = base_collection_id.as_deref() {
        if collection::get_by_id(pool, base)
            .await?
    ...
    {
        Some(id) if collection::get_by_id(pool, &id).await?.is_some() => Some(id),
        _ => None,
    };
    ...
    if existing_draft_id.is_none() {
        collection::delete_unsaved_for_game_tx(&mut tx, game_id).await?;
        ...
        collection::update_safety_summary_tx(&mut tx, &draft_id, is_safe).await?;
    }
...
pub(crate) async fn ensure_rollback_draft_is_unreferenced(
    ...
    if crate::repo::task::open_task_references_rollback_collection_tx(conn, draft_id).await? {
...
pub async fn valid_active_baseline(
    ...
    Ok(collection::get_by_id(pool, candidate_id)
        .await?
