...
use crate::services::collection::preview_tree::resolve_preview_terminal_metadata;
...
pub(crate) async fn live_runtime_is_safe(
    ...
    let unsafe_count = crate::repo::mods::count_active_unsafe_mods(pool, game_id).await?;
...
pub(crate) async fn load_game_mods_path(
    ...
    Ok(crate::repo::game::get_configured_mods_path(pool, game_id).await?)
}
...
pub(crate) async fn missing_collection_member_count_tx(
    ...
    crate::repo::collection::count_missing_mods_tx(conn, game_id, collection_id).await
}
