...
//!
//! This used to sit in `repo::object::sync`, where the resolution order,
...
use crate::repo::object::{self, ObjectIdentityRow};
...
pub async fn ensure_object_exists(
    ...
    let by_name = object::find_by_name_key(conn, input.game_id, &name_key).await?;
    let by_folder = object::find_by_folder_key(conn, input.game_id, &folder_key).await?;
    ...
    let id = object::insert_object(conn, &input).await?;
    *new_objects_count += 1;
    Ok(id)
}
...
async fn merge_into_name_match(
    ...
    if existing.folder_path != input.folder_path && !folder_taken {
        object::update_object_location(conn, &existing.id, input.folder_path).await?;
    }

    if existing.name != input.obj_name {
        object::update_object_name(conn, &existing.id, input.obj_name).await?;
    }

    if existing.object_type != input.obj_type && input.source.type_is_authoritative() {
        object::update_object_type(conn, &existing.id, input.obj_type).await?;
    }

    object::backfill_empty_columns(conn, &existing.id, input).await?;
    Ok(existing.id)
}
