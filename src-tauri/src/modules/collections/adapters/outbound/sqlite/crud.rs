...
pub async fn create(
    ...
    .await?;
    if is_draft {
        crate::modules::collections::adapters::outbound::sqlite::runtime::set_draft_tx(&mut tx, game_id, id, None).await?;
    }
    tx.commit().await?;
...
pub async fn member_safety_summary(
    ...
    for (is_safe, source) in rows {
        let is_classified = source != crate::shared::safety_constants::SAFETY_SOURCE_UNKNOWN;
