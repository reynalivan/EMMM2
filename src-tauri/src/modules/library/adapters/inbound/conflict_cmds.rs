...
async fn rollback_conflict_renames(
    state: &WatcherState,
    rewrites: &[crate::modules::library::application::mods::core_ops::FolderPathRename],
) -> Result<(), AppError> {
    let suppressor = state.suppressor.clone();
    let rewrites = rewrites.to_vec();
    tokio::task::spawn_blocking(move || {
        crate::modules::library::application::mods::core_ops::rollback_folder_conflict_renames(&suppressor, &rewrites)
    })
    .await?
}
...
pub async fn resolve_folder_name_conflict(
    ...
    let rewrites = tokio::task::spawn_blocking(move || {
        crate::modules::library::application::mods::core_ops::apply_folder_conflict_renames(
            &rename_root,
            &rename_game_id,
            &suppressor,
            &group_id,
...
pub async fn trash_folder_conflict_candidate(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        crate::modules::library::application::mods::trash::move_to_trash(&candidate_path)
    })
    .await??;
...
pub async fn ignore_object_conflict(
    ...
    crate::modules::workspace::adapters::outbound::sqlite::conflict::ignore_object_conflict(&pool, &game_id, &object_id, &mod_ids)
        .await?;
    Ok(())
}
...
pub async fn revoke_object_conflict(
    ...
    crate::modules::workspace::adapters::outbound::sqlite::conflict::revoke_object_conflict(&pool, &game_id, &object_id).await?;
    Ok(())
}
...
pub async fn list_ignored_object_conflicts(
    ...
    let list = crate::modules::workspace::adapters::outbound::sqlite::conflict::list_ignored_object_conflicts(&pool, &game_id).await?;
    Ok(list)
}
