use crate::modules::workspace::application::explorer::listing::list_mod_folders_inner;
use std::fs;
...
// Covers: TC-11-05 (Aggressive prefix parsing traps)
#[tokio::test]
async fn test_list_mod_folders_double_disabled() {
    let tmp = TempDir::new().unwrap();
    let mods = tmp.path().join("Mods");
    fs::create_dir_all(&mods).unwrap();
    fs::create_dir(mods.join("DISABLED DISABLED Skin")).unwrap();
...
// Covers: TC-11-11 (Malformed explicit metadata handling)
#[tokio::test]
async fn test_list_mod_folders_malformed_info_json() {
    let tmp = TempDir::new().unwrap();
    let mods = tmp.path().join("Mods");
    let mod_folder = mods.join("CorruptInfoMod");
    fs::create_dir_all(&mod_folder).unwrap();

    // Write malformed JSON (trailing comma, missing bracket, etc.)
    fs::write(mod_folder.join("info.json"), "{\"name\": \"Corrupt\", ").unwrap();
    ...
    // Ensure it doesn't hard crash and still lists the folder
    assert_eq!(folders[0].name, "CorruptInfoMod");
...
async fn test_get_filtered_objects_unsafe() {
    ...
    use crate::modules::catalog::adapters::outbound::sqlite::object::get_filtered_objects;
