...
pub async fn resolve_safety_targets(
    pool: &SqlitePool,
    game_id: &str,
    selected_paths: &[crate::platform::fs::guard::ValidatedPath],
    ...
    let stored_paths = crate::modules::library::adapters::outbound::sqlite::mods::get_folder_paths_for_game(pool, game_id).await?;
    ...
    for selected in selected_paths {
        ...
        for (stored_path, disk_path) in &candidates {
            ...
            if seen.insert(crate::shared::path_key::folder_path_key(
                &disk_path_string,
                None,
            )) {
...
pub async fn bulk_set_safety(
    ...
    let stored_paths = resolved
        ...
        .collect::<Vec<_>>();
    if let Err(error) =
        crate::modules::library::adapters::outbound::sqlite::mods::batch_set_safety(pool, game_id, &stored_paths, safe).await
    {
...
pub async fn bulk_toggle_favorite(
    ...
    if let Err(error) =
        crate::modules::library::adapters::outbound::sqlite::mods::batch_set_favorite(pool, &game_id, &relatives, favorite).await
    {
        let warnings = restore_info_writes(&backups);
        return Err(AppError::Io(format!(
...
pub async fn bulk_pin(
    ...
    let relatives = relative_to_mods_root(pool, &game_id, &result.success).await?;
    if let Err(error) =
        crate::modules::library::adapters::outbound::sqlite::mods::batch_set_pinned(pool, &game_id, &relatives, pin).await
    {
        let warnings = restore_info_writes(&backups);
        return Err(AppError::Io(format!(
