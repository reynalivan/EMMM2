...
pub async fn move_mods_to_object_service(
    ...
    if params.folder_paths.is_empty() {
        return Ok(OrganizerMoveOutcome {
            result: crate::modules::library::application::mods::bulk::BulkResult::new(Vec::new(), Vec::new()),
            path_hints: Vec::new(),
        });
    }

    let game_mod_path = crate::modules::games::adapters::outbound::sqlite::game::get_mod_path(pool, params.game_id)
        .await?
        .ok_or_else(|| AppError::NotFound("Game not found".to_string()))?;
    let target_obj = crate::modules::catalog::adapters::outbound::sqlite::object::get_game_object_by_id(pool, params.target_object_id)
        .await?
    ...
    for folder_path in params.folder_paths {
        ...
        .await
        {
            ...
            Err(error) => failures.push(crate::modules::library::application::mods::bulk::BulkActionError {
    ...
    Ok(OrganizerMoveOutcome {
        result: crate::modules::library::application::mods::bulk::BulkResult::with_collection_impact(
            success,
            failures,
...
async fn move_one_mod_to_object(
    ...
    let path_rewrites = vec![crate::modules::workspace::domain::workspace::WorkspacePathRewrite {
        old_path: old_rel.clone(),
        new_path: new_rel.clone(),
    }];
