...
use crate::modules::workspace::application::scanner::deep_matcher::{EntryKind, MasterDb};
...
pub async fn commit_import_batch(
    ...
    crate::modules::workspace::application::disk_reconcile::emit::ensure_mutation_preflight(app, pool, &batch.game_id)
        .await?;
...
async fn resume_committed_items(
    ...
    if let Some(active_lease) = lease.as_ref() {
        ...
        let hints = items
            ...
            .collect();
        let reconcile = crate::modules::workspace::application::disk_reconcile::emit::run_internal_disk_reconcile_with_path_hints_under_lease(
            app,
            pool,
            &batch.game_id,
            changed_paths,
