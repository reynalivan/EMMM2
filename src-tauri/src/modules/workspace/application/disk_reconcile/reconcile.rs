...
use crate::modules::workspace::application::disk_reconcile::disk_snapshot::{
...
use crate::modules::workspace::application::disk_reconcile::orchestrator::DiskReconcilePathHint;
use crate::modules::workspace::application::disk_reconcile::path_classifier::{
    collect_changed_roots, collect_thumbnail_roots, is_runtime_relevant_file,
};
use crate::modules::workspace::application::disk_reconcile::projection_writer::{
    reconcile_projection_in_tx, ProjectionWriteRequest,
};
use crate::modules::workspace::application::disk_reconcile::rename_confirmation::detect_rename_confirmations;
use crate::modules::workspace::application::disk_reconcile::rename_healer::{
    apply_watcher_rename_hints, WatcherRenameHintsApplyRequest,
};
use crate::modules::workspace::application::disk_reconcile::types::{
...
pub struct ReconcileOutcome {
    ...
    pub rename_confirmations: Vec<crate::modules::workspace::application::disk_reconcile::types::RenameConfirmationGroup>,
...
pub struct ReconcileDiskProjectionRequest<'a> {
    ...
    pub progress_reporter:
        Option<Arc<crate::modules::workspace::application::disk_reconcile::orchestrator::DiskReconcileProgressReporter>>,
}
...
pub async fn reconcile_disk_projection(
    ...
    for changed_path in changed_paths {
        let path = Path::new(changed_path);
        if crate::modules::workspace::application::disk_reconcile::path_classifier::is_thumbnail_path(path) {
    ...
    let thumbnail_only_watcher_batch = matches!(reason, DiskReconcileReason::WatcherBatch)
        ...
        && changed_paths.iter().all(|path| {
            crate::modules::workspace::application::disk_reconcile::path_classifier::is_thumbnail_path(Path::new(path))
        });
    ...
    if should_reconcile {
        ...
        let snapshot = tokio::task::spawn_blocking(move || {
            let on_progress = |progress: DiskSnapshotProgress| {
                if let Some(reporter) = &snapshot_progress {
                    reporter.emit(
                        crate::modules::workspace::application::disk_reconcile::types::DiskReconcilePhase::ScanningRoots,
        ...
        if let Some(reporter) = &progress_reporter {
            reporter.emit(
                crate::modules::workspace::application::disk_reconcile::types::DiskReconcilePhase::Projecting,
                0,
                None,
                None,
            );
        ...
        let projection = crate::modules::workspace::application::disk_reconcile::disk_snapshot::DiskProjection {
            objects: projection
                .objects
                .iter()
