...
use crate::modules::workspace::application::disk_reconcile::disk_snapshot::{collect_disk_projection, DiskProjection};
...
pub struct ApplyGameModsDirectoryResult {
    pub game: crate::modules::system::application::config::GameConfig,
    pub inspection: GameModsDirectoryInspection,
    pub reconcile: crate::modules::workspace::application::disk_reconcile::types::DiskReconcileResult,
}
...
pub async fn apply_game_mods_directory(
    context: crate::modules::workspace::application::disk_reconcile::orchestrator::DiskReconcileContext<'_>,
    ...
    let reconcile_result =
        crate::modules::workspace::application::disk_reconcile::orchestrator::reconcile_disk_state_under_locks(
            context.clone(),
    ...
    let reconcile = match reconcile_result {
        Ok(result)
            if result.status
                != crate::modules::workspace::application::disk_reconcile::types::DiskReconcileStatus::SourceUnavailable =>
        {
            result
        }
        Ok(result) => {
...
mod tests {
    ...
    async fn apply_rejects_a_candidate_changed_after_inspection_without_updating_config() {
        ...
        let config = crate::modules::system::application::config::ConfigService::new_for_test(context.pool.clone());
    ...
    async fn post_commit_runtime_effect_failure_does_not_restore_the_previous_source() {
        ...
        assert!(reconcile.warnings.iter().any(|warning| {
            warning.kind
                == crate::modules::workspace::application::disk_reconcile::types::DiskReconcileWarningKind::RuntimeEffectsPending
        }));
    }
}
