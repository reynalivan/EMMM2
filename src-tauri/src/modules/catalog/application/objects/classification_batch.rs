...
pub async fn preview_object_classification_batch(
    ...
    let mods_root = crate::modules::games::adapters::outbound::sqlite::game::get_mod_path(db, &input.game_id)
        .await?
...
pub async fn apply_object_classification_batch(
    ...
    for item in input.items {
        let object = crate::modules::catalog::adapters::outbound::sqlite::object::get_game_object_by_id(db, &item.object_id)
            .await?
            .filter(|object| object.game_id == input.game_id)
