...
async fn ready_to_move_archive_failure_stays_resumable_until_processed_move_succeeds() {
    ...
    .await
    .unwrap();
    let pending = crate::modules::ingestion::adapters::outbound::sqlite::import_batch::get_batch(&context.pool, "batch-archive")
        .await
        .unwrap()
        .unwrap();
    ...
    finalize_ready_to_move_archives(&context.pool, "batch-archive")
        .await
        .unwrap();
    crate::modules::ingestion::adapters::outbound::sqlite::import_batch::complete_ready_to_move_archive_pending(
        &context.pool,
        "batch-archive",
    )
    .await
    .unwrap();
    assert_eq!(
        crate::modules::ingestion::adapters::outbound::sqlite::import_batch::finish_batch_from_items(&context.pool, "batch-archive",)
            .await
            .unwrap(),
