...
#[test]
fn test_mod_ini_with_meaningful_children_is_modpack() {
    let tmp = TempDir::new().unwrap();
    let dir = tmp.path().join("RaidenShogun");
    fs::create_dir(&dir).unwrap();
    ...
    // Create mod asset
    fs::write(dir.join("RaidenBody.buf"), "fake buffer data").unwrap();
    // Create a meaningful child directory
    fs::create_dir(dir.join("Variants")).unwrap();
...
// Covers: TC-11-16 (Classification priority rule)
#[test]
fn test_classification_priority_rule() {
    let tmp = TempDir::new().unwrap();
    let dir = tmp.path().join("FrankensteinMod");
    fs::create_dir(&dir).unwrap();

    // 1. Matches ModPackRoot (has ini and assets)
    fs::write(dir.join("main.ini"), "[TextureOverrideMain]\nhash = abc\n").unwrap();
    fs::write(dir.join("asset.buf"), "binarydata").unwrap();

    // 2. Add 5 sub-folders with valid inis (Matches VariantContainer logic)
    for i in 0..5 {
        let child = dir.join(format!("Variant{}", i));
        fs::create_dir(&child).unwrap();
        fs::write(
            child.join("variant.ini"),
            format!("[TextureOverrideV{}]\nhash = hash{}", i, i),
        )
        .unwrap();
    }

    // 3. Add random raw .txt files (ContainerFolder logic)
    fs::write(dir.join("readme.txt"), "hello").unwrap();

    let (node_type, _reasons) = classify_folder(&dir);
    // Because it has mod assets + meaningful children (Variant dirs are meaningful),
    // it MUST correctly resolve to ModPackRoot instead of returning early as a VariantContainer.
    assert_eq!(node_type, NodeType::ModPackRoot);
}
