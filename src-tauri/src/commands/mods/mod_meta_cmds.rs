...
fn restore_info_json(path: &std::path::Path, previous: Option<&[u8]>) -> Result<(), AppError> {
    match previous {
        Some(bytes) => crate::services::fs_utils::atomic_file::atomic_write(path, bytes),
...
pub async fn update_mod_info(
    ...
    drop(guard);
    drop(lock);
    let reconcile = crate::services::disk_reconcile::emit::run_internal_disk_reconcile(
        &app,
        pool.inner(),
        &game_id,
        vec![changed_path],
    ...
    if let Some(failure) = failure {
        ...
        drop(rollback_guard);
        drop(rollback_lock);
        let repair = crate::services::disk_reconcile::emit::run_full_internal_disk_reconcile(
            &app,
            pool.inner(),
            &game_id,
        )
...
pub async fn set_mod_category(
    ...
    drop(game_guard);
    let runtime_effects = crate::services::app::runtime_effects::settle_committed_runtime_effects(
...
pub async fn set_object_mods_category(
    ...
    let game_guard = game_lock.lock().await;
    let operation_guard = op_lock.acquire_for_reconcile().await;
    let updated = crate::services::objects::mutate::set_object_and_mods_category(
        pool.inner(),
        &game_id,
        &object_id,
        &category,
...
pub async fn move_mods_to_object(
    ...
    let mutation_lease = disk_reconcile_state
        ...
        .await?;
    let organizer = crate::services::mods::organizer_ext::move_mods_to_object_service(
        pool.inner(),
    ...
    if let Some(reconcile) = settlement.reconcile {
        ...
        for update in reconcile.path_updates {
            if !result.path_rewrites.iter().any(|rewrite| {
                ...
                result
                    .path_rewrites
                    .push(crate::domain::workspace::WorkspacePathRewrite {
