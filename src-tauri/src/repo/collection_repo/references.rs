...
pub async fn stage_member_path(
    ...
    .bind(staged_path)
    .bind(crate::common::path_key::folder_path_key(staged_path, None))
    .bind(old_path)
    .bind(staged_path)
...
pub async fn finalize_staged_member_path(
    ...
    let final_key = crate::common::path_key::folder_path_key(final_path, None);
    ...
    let matching_destination = destination_paths
        .iter()
        .find(|path| crate::common::path_key::folder_path_key(path, None) == final_key);
...
pub async fn update_member_paths(
    ...
    let new_path_key = crate::common::path_key::folder_path_key(new_mod_path, None);
    ...
    for collection_id in source_collections {
        ...
        if let Some(destination_path) = sibling_paths
            .iter()
            .find(|path| crate::common::path_key::folder_path_key(path, None) == new_path_key)
        {
            merged +=
                sqlx::query("DELETE FROM collection_mods WHERE collection_id = ? AND mod_path = ?")
                    .bind(&collection_id)
    ...
    .bind(new_path_key)
    .bind(
        crate::common::path_key::folder_path_key(new_mod_path, None)
            .split('/')
            .next()
            .unwrap_or_default(),
    )
...
pub async fn rewrite_member_path(
    ...
    .bind(
        crate::common::path_key::folder_path_key(new_path, None)
            .split('/')
            .next()
            .unwrap_or_default(),
    )
