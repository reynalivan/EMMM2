...
- **Catalog Pack Updates:** The Catalog adapter checks only the fixed public GitHub release channel without end-user credentials. It downloads `catalog-pack.zip` plus its Ed25519 signature, validates both a bounded archive and staged data-only pack, atomically swaps the app-data `asset-pack`, then invalidates MasterDB cache and refreshes only asset-pack-derived thumbnails. The private signing key remains outside EMMM and Git.
