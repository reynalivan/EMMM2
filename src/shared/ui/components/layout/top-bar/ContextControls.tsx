...
import { useCollectionRuntimeDescriptor } from '@/pages/collections/hooks/useCollectionRuntime';
...
import { SaveCollectionModal } from '@/pages/collections/components/SaveCollectionModal';
import { ApplyCollectionModal } from '@/pages/collections/components/ApplyCollectionModal';
      ...
      <div className="hidden lg:flex items-center gap-3 bg-base-100/30 p-1.5 rounded-full border border-base-content/10 backdrop-blur-md">
        <div className="dropdown dropdown-bottom dropdown-end">
          <div
            tabIndex={0}
            role="button"
            className={`px-3 py-1 rounded-full text-xs font-medium text-base-content/80 hover:text-base-content hover:bg-base-content/10 transition-all cursor-pointer flex items-center gap-2 max-w-37.5 ${activeNamedCollectionId === null && (collections || []).length > 0 ? 'italic opacity-90' : ''}`}
            title={triggerText}
          >
          ...
          <ul
            tabIndex={0}
            className="dropdown-content z-50 menu p-2 shadow-2xl bg-base-100/95 backdrop-blur-xl rounded-box w-56 mt-2 border border-base-content/10"
          >
