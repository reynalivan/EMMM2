...
export default function EditObjectModal({ open, object, onClose }: EditObjectModalProps) {
  ...
  // Hydrate auto-sync selection once masterDb is available
  useEffect(() => {
    if (open && activeTab === 'auto' && masterDb && !selectedSyncEntry) {
      const defaultName = form.getValues('name');
      if (defaultName) {
        const allEntries = Object.values(masterDb).flat();
        const exactMatch = allEntries.find((e) => e.name === defaultName);
        if (exactMatch) {
          const t = setTimeout(() => {
  ...
  }, [open, activeTab, masterDb, selectedSyncEntry, form, setDbSearch]);
