/**
 * useScanReviewFlow — owns the scan review modal state and the two ways it gets
 * filled: an explicit Deep Match scan, and Disk Reconcile repair.
 *
 * The import flow (drop + archive) writes into the same state through the
...
import type { ConfirmedScanItem } from '../../../types/scanner';
...
import { CLOSED_SCAN_REVIEW, type ScanReviewState } from '../utils/importPipeline';

export function useScanReviewFlow() {
  ...
  const [scanReview, setScanReview] = useState<ScanReviewState>(CLOSED_SCAN_REVIEW);
