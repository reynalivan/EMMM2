...
describe('Fase 4b — Editor Edges', () => {
  ...
  it('TC-17-02: Tags add then remove round-trips via metadata update', async () => {
    ...
    let info = await invokeInApp<ModInfo>('read_mod_info', { folderPath: modDir });
    ...
    info = await invokeInApp<ModInfo>('read_mod_info', { folderPath: modDir });
    expect(info.tags).toContain('favorite');
  ...
  it('TC-18-02: Multiple INI files are listed and each is readable', async () => {
    ...
    expect(files).toContain('extra.ini');

    for (const fileName of files) {
      const doc = await invokeInApp('read_mod_ini', { folderPath: modDir, fileName });
