...
> **[2026-08-09] Dokumen historis.** Log fase refactor 2026-07-26, sudah selesai dan kemudian DILAMPAUI oleh refactor disk-truth 2026-08-09 (`docs/knowledge/enable-disable-flow.md`): `runtime_mutation_engine` kini rename-only (commit_db dihapus), status/`folder_path` ditulis hanya oleh Disk Reconcile, `disabled_reason` di-drop. Angka baris dan klaim arsitektur di bawah adalah snapshot lama.
...
- Reuse: `test_utils.rs`, `tests/e2e/support/*`, pola audit-test yang ada.
