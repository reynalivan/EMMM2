...
- Active 500ms debounce drops rapid sequential keystrokes. -`switch_lock` mechanism actively drops inputs if a previous hotkey operation is currently generating artifacts or applying workspace.
...
**Requirement File**:`e:\Dev\EMMMNEW\docs\requirements\req-42-ingame-hotkeys.md`
...
| AC-42.1.5 (EMMM closed = inactive)          | TC-42-005         |
...
| :-------- | :-------------------------------------- | :------- | :------- | :--------------- | :--------------------------------------------------------------------------------------------------- | :-------------------------------------------- | :--------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------- |
| TC-42-001 | Hotkey intercepted while app minimized  | Positive | High     | S1               | Game is running and in foreground. EMMM is minimized to tray.`F5` bound to Safe Mode.                | Hotkey:`F5`                                   | 1. Press`F5` while focused on the game window.                                     | EMMM backend captures`F5`. Safe mode is toggled.`runtime_status.txt` is updated. Game reload key is dispatched.                                                                                      | AC-42.1.1 |
| TC-42-002 | Hotkey dropped when game not focused    | Negative | High     | S1               | EMMM is running. Desktop or browser is the active 100% foreground window.                            | Foreground: chrome.exe                        | 1. Press`F5` in the browser window.                                                | EMMM logs "hotkey discarded: foreground check failed". Safe mode status is untouched. Original keystroke passes through accurately to the browser.                                                   | AC-42.1.2 |
...
| TC-42-005 | App close clears global hook            | Positive | High     | S2               | EMMM is closed. Game is running.                                                                     | EMMM terminated.                              | 1. Press`F5` in game.                                                              | No action is triggered because the background listener thread`tauri-plugin-global-shortcut` is dead. Game executes native default action for`F5`.                                                    | AC-42.1.5 |
...
| TC-42-013 | Phase 5: Banner textual format write    | Positive | Medium   | S2               | File watcher targeted at`.emmm_data/generations/status/runtime_status.txt`.                         | Action executed.                              | 1. Execute any hotkey action.                                                      | Backend writes the stable status resource through a temporary sibling and atomic rename, injecting raw UTF-8 banner text without exposing a partial file to 3DMigoto.             | Phase 5   |
...
- When`AUTO_SCOPE_FROM_GAME = ON`, the scope is read from`$kv_active_code` in`d3dx_user.ini`. What if this file is locked by 3DMigoto when EMMM reads it mid-render? Is there a retry strategy?
...
- **Game**:`GenshinImpact.exe` configured in EMMM with valid`mods_path` and`d3dx.ini`
- **DB State**:`emmm.db` with 3 Collections (`A`,`B`,`C`) and mods with`is_safe` flags set
- **Status Path**:`Mods/.emmm_data/generations/status/runtime_status.txt` (must exist as writable path)
