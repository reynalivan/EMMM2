...
- Phase 5: 6 explicit EMMM Event Triggers accurately command atomic`.ini` regeneration without blocking.

## B. Coverage Matrix

**Requirement File**:`e:\Dev\EMMMNEW\docs\requirements\req-43-dynamic-keyviewer.md`
...
| TC-43-004 | Output Artifacts Atomic Write         | Positive | High     | S1               | Normal mapping output to the stable resource directory.                                    | `Mods/.emmm_data/generations/` | 1. Force regeneration.<br>2. Monitor filesystem IO sequentially.                                                                                                           | System stages the complete `generations/` tree, swaps it atomically, and keeps one active directory with no versioned children for 3DMigoto to scan. | AC-43.3.1 |
...
| TC-43-007 | Phase 5: Ambiguous Hash Blacklist     | Edge     | High     | S1               | A shader hash effectively maps to 5 unique characters simultaneously.                       | Blacklisted`hash`.             | 1. EMMM accurately detects multi-character intersection logic.<br>2. It accurately maps the physical hash to`blacklisted_for_sentinel: true`.<br>3. Parses`KeyViewer.ini`. | The blacklisted hash contributes to detection but is excluded from valid `[TextureOverrideKV_*]` sentinels.                                     | Phase 5   |
...
- **Elevated Contexts**: Does the`enigo` keypress synthesize strokes into the Game window if the Game is running strictly As Administrator but EMMM is running natively as Standard User? (Likely blocked by Windows UIPI - needs fallback documentation).
...
- **KeyViewer Output Paths**: -`Mods/.emmm_data/KeyViewer.ini` -`Mods/.emmm_data/generations/keybinds/active/{code_hash}.txt` -`Mods/.emmm_data/generations/keybinds/active/_fallback.txt`
- **3DMigoto**: Running
- **KeyViewer Feature Toggle**: ON in EMMM settings
