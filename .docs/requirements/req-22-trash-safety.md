...
delete_mod(game_id, folder_path) → Result<(), CommandError>:
  ...
  3. uuid = generate()
  4. fs::rename(folder_path, {app_data_dir}/trash/{uuid}/mod_folder)
  5. fs::write({app_data_dir}/trash/{uuid}/metadata.json)
  6. DELETE FROM folders WHERE folder_path = ?
  7. Return Ok(())

restore_mod(game_id, trash_uuid) → Result<(), CommandError>:
  1. Load metadata from {app_data_dir}/trash/{trash_uuid}/metadata.json
  2. Check original_path.exists() → Error if true
  3. fs::rename({app_data_dir}/trash/{trash_uuid}/mod_folder, original_path)
  4. fs::remove_dir_all({app_data_dir}/trash/{trash_uuid}/)
  5. Return Ok(())
```
