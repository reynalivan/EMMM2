...
| ID        | Type        | Criteria                                                                                                                                                                                                          |
| --------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-41.1.1 | ✅ Positive | Given a mod folder containing `preview.png` or `preview.jpg`, when the grid requests a thumbnail, then the backend generates a 256×256 WebP, stores it to disk, and returns the absolute disk path within ≤ 200ms |
...
| ID        | Type        | Criteria                                                                                                                                                                    |
| --------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-41.2.1 | ✅ Positive | Given a thumbnail is generated, then it is saved to `{app_data_dir}/thumbnails/{blake3(folder_path)}.webp`; the L1 `RwLock<HashMap>` is updated with the absolute disk path |
| AC-41.2.2 | ✅ Positive | Given a repeated request for the same folder (fast scroll, re-mount), then the L1 HashMap returns the path in ≤ 1ms — no disk read, no re-encode                            |
| AC-41.2.3 | ✅ Positive | Given the source image's `mtime` has changed since last cache entry, then the L1 entry is invalidated, the WebP is re-generated and the new absolute path is served         |
...
ThumbnailCache state (Tauri managed):
  l1: Arc<RwLock<HashMap<String, CacheEntry>>>
    CacheEntry { webp_path: String, source_mtime: SystemTime }
...
get_or_generate_thumbnail(folder_path: PathBuf) → Option<String>:
  ...
  6. webp_path = (thumbnails_dir / "{hash_key}.webp").canonicalize().to_string()
  7. l1.write().insert(hash_key, CacheEntry { webp_path, source_mtime })
  8. Return Some(webp_path)
...
- **Backend returns absolute paths** validated with `canonicalize()` — no arbitrary path serving from unvalidated input.
- **Tauri `asset://` scope** is set to `["**"]` in `tauri.conf.json` to support mod assets outside AppData.
- **Safe Mode respected**: If `safeMode && !mod.is_safe`, the frontend renders the blur placeholder icon — the thumbnail path is never passed to `convertFileSrc`.
