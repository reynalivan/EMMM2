...
- **Success Criteria**: -`list_mod_ini_files` completes in ≤ 100ms.
- Content loads in ≤ 200ms for ≤ 500KB files.
- Custom CodeMirror 6 highlighting captures 95% of 3DMigoto syntax. -`write_mod_ini` saves in ≤ 300ms, using`WatcherSuppression`.
...
**Requirement File**:`e:\Dev\EMMMNEW\.docs\requirements\req-18-ini-viewer.md`
...
- **External File Changes**: What happens when an INI is edited in Notepad++ and saved while open in EMMM? (Implied: File watcher event invalidates the cache, Editor should prompt to reload or auto-reload if not dirty).
...
- **Preconditions**: OS: Windows 10/11. App: EMMM dev build. File Watcher active. CodeMirror syntax package installed.
- **Context Injection**:
