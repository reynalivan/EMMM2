...
- **Feature Goal**: Run a live background filesystem monitor on`mods_path` that detects external creations, deletions, and renames, instantly pushing`disk_reconcile:result` payloads to the frontend while suppressing internal operations.
...
- Internal bulk operations suppressed internally (0 watcher-sourced `disk_reconcile:result` emitted).
...
| :------------------------------- | :---------------- | :--------------------------------------------------------- |
| AC-28.1.1 (External Create)      | TC-28-001         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.1.2 (External Delete)      | TC-28-002         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.1.3 (External Rename)      | TC-28-003         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.1.4 (Root Deleted)         | TC-28-004         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.2.1 (Internal Suppression) | TC-28-005         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.2.2 (Panicked Scope Drop)  | TC-28-006         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| AC-28.2.3 (Colliding Events)     | TC-28-007         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
| N/A (Game Switch Watcher)        | TC-28-008         | `e:\Dev\EMMMNEW\.docs\requirements\req-28-file-watcher.md` |
...
| TC-28-005 | Suppress internal operation        | Positive | High     | Bulk rename execution on 100 mods                                 | 1. EMMM internal Bulk operation running.<br>2. Run an internal action that generates many`fs::rename`.               | Watcher emits 0 watcher-sourced `disk_reconcile:result` events for those paths.                                        | S1               | AC-28.2.1 |
...
| TC-28-007 | Colliding external/internal change | Edge     | Low      | External user changes exact same path as EMMM at same millisecond | 1. Suppressed internal renaming running.<br>2. User modifies exact same path via OS concurrently.                    | The watcher event is suppressed; the next refocus/manual Disk Reconcile repairs the projection.           | S3               | AC-28.2.3 |
...
- **E2E-28-01 (External Modification vs Grid Update)**: The user initiates Mod organization via Windows Explorer, adding 15 new archives (Epic 28). The File Watcher detects these structural updates and pushes debounced `disk_reconcile:result` payloads to the Tauri window. The frontend receives the event and explicitly invalidates the React Query cache for the affected paths, causing the Folder Grid to update without a full application refresh.
