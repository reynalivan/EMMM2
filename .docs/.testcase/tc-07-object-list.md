...
- Toggling mod updates objectlist counts ≤50ms after the switch result publishes its refresh (invalidation-only).
...
**Requirement File**:`e:\Dev\EMMMNEW\.docs\requirements\req-07-object-list.md`
...
| TC-07-07 | Fast-refresh active badges        | Positive | High     | `Active Folders`                         | 1. Navigate to Grid housing multiple mods.<br>2. Click 'Enable' specifically on a Mod.<br>3. Observe ObjectList counts immediately.     | ObjectList`Enabled/Total` tag updates ≤50ms after the toggle result lands (refetch-driven; no local mutation of counts).                            | S3               | AC-07.3.1, AC-07.3.2            |
...
- **Resolved**: Dragging visually uses the native OS-level drag overlay because EMMM uses Tauri's physical file dragging (`tauri://drag-enter`), abandoning the React-dom limited`dnd-kit`. Auto-scrolling on container edges is natively handled via`useDragAutoScroll` hook explicitly for massive virtualized lists.
