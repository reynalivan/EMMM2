...
- **Feature Goal**: Enable or disable a mod by adding/removing the`DISABLED` prefix on the physical folder, utilizing an invalidation-only UI (pending state + refetch),`OperationLock`, and path-scoped watcher suppression. Collision detection acts as a safety against silently overwriting folders.
- **User Roles**: End User
- **User Story**:
- US-20.1: Toggle Individual Mod
- US-20.2: Handle Toggle Collisions
- **Success Criteria**:
- Pending state shows ≤ 16ms; refetched state lands after the scoped reconcile.
- ObjectList`enabled_count` updates ≤ 50ms after the switch result publishes its refresh.
...
**Requirement File**:`e:\Dev\EMMMNEW\.docs\requirements\req-20-mod-toggle.md`
...
| -------- | ----------------------------- | -------- | -------- | ------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------- | -------------------- |
| TC-20-01 | Toggle Basic Behavior         | Positive | High     | `N/A`                     | 1. Select Mod`DISABLED MyMod` in grid.<br>2. Click toggle switch.                                   | Mod is renamed to`MyMod`. Card shows pending in ≤ 16ms and the refetched state lands after the reconcile.                    | S2               | AC-20.1.1, AC-20.1.2 |
...
| TC-20-03 | OperationLock Timeout         | Negative | Med      | `External rename ongoing` | 1. Force backend lock externally.<br>2. Attempt Toggle click in UI.                                 | Lock Wait times out (3s). Returns "Operation in progress". Pending state clears; the switch still shows its refetched (unchanged) state.                                   | S2               | AC-20.1.4            |
| TC-20-04 | Rapid Spamming Endurance      | Positive | High     | `Mod file`                | 1. Spam click the toggle 10 times in 1 second.                                                      | Frontend pending-state debounce handles intent. Backend runs exactly one conclusive rename under the lock, avoiding file corruption.    | S1               | AC-20.1.5            |
...
- **TC-20-01 (Toggle pending state + refetch)**: Essential feature functionality test (Vitest frontend mutation integration).
...
- **Preconditions**: OS: Windows 10/11. App: EMMM dev build. Object Panel and File Watcher fully active.
- **Context Injection**:
