...
- **Acceptance Criteria**: -`suggest_random_mods` returns instantly`<200ms` avoiding conflicting selections (2 mods selected simultaneously over 1 character).
...
**Requirement File**:`e:\Dev\EMMMNEW\.docs\requirements\req-35-smart-randomizer.md`
...
| TC-35-002 | Auto-Close EMMM Logic            | Positive | Medium   | S2               | "Auto-Close after launch" Setting is enabled.                            | Settings flag`auto_close`.     | 1. Click "Play".                                                 | Application invokes external exe processes and calls Tauri`app.exit(0)` securely closing the mod manager entirely.                                             | AC-35.1.2 |
...
| TC-35-013 | System Folder Dot Exclusion      | Positive | Medium   | S2               | `.EMMM_System` physical folder exists mapping system mods.               | Dot-prefixed folder.           | 1. Check generated pools.                                        | Filter ignores strict dot-folders.                                                                                                                             | AC-35.3.1 |
...
- **Database Objects**: Populate`folders` with Characters: "Albedo" (5 Mods), "Raiden" (2 Mods - Both NSFW), "Hu Tao" (1 Mod). Include`.EMMM_System` as a simulated dot-prefix locked object mapping.
