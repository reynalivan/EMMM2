...
- Modularity: Keep modules cohesive; split files only when responsibilities, state, or control flow become difficult to reason about, never because of a line-count threshold.
- SRP: Each function has one clear task; extract only a genuinely independent responsibility. snake_case (Rust/Files), PascalCase (Components/Types), camelCase (FE).
